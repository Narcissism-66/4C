...
// 获取用户信息，用于生成存储 key（确保 UserStore 正常引入）
const userStore = userUserStore();
const storageKey = `GraphChartConfig-${userStore.user.id}`;
...
const closeDialog = () => {
  ...
  updateChart();
  saveGraphConfigLocally();
}

// 保存配置到 localStorage
const saveGraphConfigLocally = () => {
  const config = {
    graphTitle: graphTitle.value,
    titleColor: titleColor.value,
    titlePosition: titlePosition.value,
    repulsion: repulsion.value,
    tooltipBgColor: tooltipBgColor.value,
    tooltipTextColor: tooltipTextColor.value,
    tooltipFontSize: tooltipFontSize.value,
    linkColor: linkColor.value,
    linkWidth: linkWidth.value,
    linkCurve: linkCurve.value
  }
  localStorage.setItem(storageKey, JSON.stringify(config));
}

// 从 localStorage 加载配置
const loadGraphConfigLocally = () => {
  const configStr = localStorage.getItem(storageKey);
  if (configStr) {
    try {
      const config = JSON.parse(configStr);
      graphTitle.value = config.graphTitle || graphTitle.value;
      titleColor.value = config.titleColor || titleColor.value;
      titlePosition.value = config.titlePosition || titlePosition.value;
      repulsion.value = config.repulsion || repulsion.value;
      tooltipBgColor.value = config.tooltipBgColor || tooltipBgColor.value;
      tooltipTextColor.value = config.tooltipTextColor || tooltipTextColor.value;
      tooltipFontSize.value = config.tooltipFontSize || tooltipFontSize.value;
      linkColor.value = config.linkColor || linkColor.value;
      linkWidth.value = config.linkWidth || linkWidth.value;
      linkCurve.value = config.linkCurve || linkCurve.value;
    } catch (error) {
      console.error("加载关系图配置失败", error);
    }
  }
}
