...
// 定义存储 key（如需要结合用户信息，可在此处动态生成）
const userStore = userUserStore()
const storageKey = `RadarChartConfig-${userStore.user.id}`
...
const saveChartConfigLocally = () => {
  const config = {
    ...
    radarAxisLineColor: radarAxisLineColor.value,
    radarSplitLineColor: radarSplitLineColor.value,
    radarSplitAreaColor: radarSplitAreaColor.value,
    indicatorLabelColor: indicatorLabelColor.value,
    seriesLineColor: seriesLineColor.value,
    seriesAreaColor: seriesAreaColor.value
  }
...
const loadChartConfigLocally = () => {
  ...
  if (configStr) {
    try {
      ...
      radarAxisLineColor.value = config.radarAxisLineColor || radarAxisLineColor.value
      radarSplitLineColor.value = config.radarSplitLineColor || radarSplitLineColor.value
      radarSplitAreaColor.value = config.radarSplitAreaColor || radarSplitAreaColor.value
      indicatorLabelColor.value = config.indicatorLabelColor || indicatorLabelColor.value
      seriesLineColor.value = config.seriesLineColor || seriesLineColor.value
      seriesAreaColor.value = config.seriesAreaColor || seriesAreaColor.value
    } catch (error) {
      console.error("加载雷达图配置失败", error)
    }
  }
}
